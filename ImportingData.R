...
# verify rio and curl installation 
library(curl)
library(rio)
...
# CSV
rio_csv <- import("~/R tutorials/ImportingData_Datasets/mbb.csv")
head(rio_csv)

# TXT
rio_txt <- import("~/R tutorials/ImportingData_Datasets/mbb.txt")
head(rio_txt)

# Excel XLSX
rio_xlsx <- import("~/R tutorials/ImportingData_Datasets/mbb.xlsx")
head(rio_xlsx)
...
# complete data.
r_txt1 <- read.table("~/R tutorials/ImportingData_Datasets/mbb.txt", header = TRUE)

# This works with missing data by specifying the separator: 
# \t is for tabs, sep = "," for commas. R converts missing
# to "NA"
r_txt2 <- read.table("~/R tutorials/ImportingData_Datasets/mbb.txt", 
  header = TRUE, 
  sep = "\t")
...
trends.csv <- read.csv("~/R tutorials/ImportingData_Datasets/mbb.csv", header = TRUE)
